	...
	switch (category) {
		...
		case "eu":
			return cities.filter((city) =>
				["FR", "NO", "SE", "AT", "IT", "FI", "CH"].some((code) =>
					city.cityname.endsWith(code)
				)
			);
		case "uc":
			return cities.filter((city) =>
				["US", "CA"].some((code) => city.cityname.endsWith(code))
			);
		default:
			return [];
	}
}
const fetchingDate = new Date();

let todaysDate = fetchingDate.getDate();
let todaysMonth = fetchingDate.getMonth() + 1;
let currentDate = `${todaysDate}/${todaysMonth}`;
...
<template>
	<div class="displayForecast">
		<label for="forecastDays" class="ice"
			>How many days forecast would you like to see?</label
		>
		...
		<ul>
			<li v-for="(city, cityIndex) in weatherData" :key="cityIndex">
				...
				<ul>
					<li
						v-for="(day, dayIndex) in city.weather.daily.slice(0, forecastDays)"
						:key="dayIndex"
					>
						<span v-if="day.snow > 0">❄️</span>
						<span v-else class="emoji-space"> &nbsp; </span>
						Date: {{ `${todaysDate + dayIndex + 1}  / ${todaysMonth} ` }} - The
						max temp will be
						<span class="ice">
							{{
								weatherData[cityIndex].weather.daily[dayIndex].temp.max + "°C."
							}}</span
						>
						{{
							weatherData[cityIndex].weather.daily[dayIndex]?.summary ||
							"no summary found"
						}}
						& <span class="ice">{{ day.snow || 0 }} cm </span> snow will fall.
					</li>
				</ul>
			</li>
		</ul>
